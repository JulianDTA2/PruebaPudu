...
function NavItem({ to, children, end = false, onClick, className = "" }) {
  return (
    <NavLink
      ...
      className={({ isActive }) => `btn-neu shrink-0 ${isActive ? "ring-2 ring-brand-pudu" : ""} ${className}`}
      onClick={onClick}
    >
      <span>{children}</span>
    </NavLink>
      ...
      <header className="sticky top-0 z-[60] p-4 space-y-4">
        <div className="max-w-7xl mx-auto">
          <div className="card-neu isolate flex overflow-x-auto whitespace-nowrap snap-x snap-mandatory items-center gap-3">
            <img width={100} src="https://www.pudurobotics.com/_next/static/media/logo.851bf515.svg" />
            <div>|</div>
            <img width={100} src="https://roboticminds.com.ec/registro/wp-content/uploads/2024/12/Sin-titulo-5-scaled.png" />
          ...
          <div className="max-w-7xl mx-auto">
            <div className="card-neu-lg isolate mt-2">
              <div className="flex flex-col gap-3">
                <NavItem to="/admin" end onClick={() => setOpen(false)}>Dashboard</NavItem>
                <NavItem to="/admin/maps" onClick={() => setOpen(false)}>Mapas</NavItem>
                <NavItem to="/admin/apicc" onClick={() => setOpen(false)}>CC1</NavItem>
                <NavItem to="/admin/bellabot" onClick={() => setOpen(false)}>BellaBot</NavItem>
                <NavItem to="/admin/logs" onClick={() => setOpen(false)}>Logs</NavItem>
                <NavItem to="/admin/explorer" onClick={() => setOpen(false)}>API</NavItem>
              </div>
            </div>
          </div>
        </div>
